...
class Auth
{
 ...
 $stmt = $this->db->prepare("INSERT INTO tblkoneksi(nama, email, password)
VALUES(:nama, :email, :pass)");
 ...
 $stmt = $this->db->prepare("SELECT * FROM tblkoneksi WHERE email = :email");
 $stmt->bindParam(":email", $email);
 $stmt->execute();
 $data = $stmt->fetch();
 // Jika jumlah baris > 0
 ...
 $stmt = $this->db->prepare("SELECT * FROM tblkoneksi WHERE id = :id");
